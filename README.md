...
<video src="./policy_run.mp4" controls width="100%"></video>

[Демо политики: policy_run.mp4](./policy_run.mp4)
...
почти невозможно нащупать случайным исследованием, и reward держится на уровне
штрафа за время (`ep_len≈300`, timeout). Рабочий путь — **warm-start
демонстрациями скриптового эксперта** (reset-free / SAC-from-Demonstrations):

1. **Скриптовый эксперт** (`src/robozone/expert.py`) — конечный автомат на IK,
   выдаёт действия в формате среды (~74% успеха).
2. **Сбор демонстраций** (`collect_demos.py`) — переходы успешных эпизодов.
3. **Behavior Cloning** актёра SAC на демонстрациях. Ключевой нюанс: измерение
   вакуума бинарное — при обычном MSE усредняется к «выкл» и политика не
   захватывает; поэтому оно взвешено ×5. BC в одиночку даёт **~72–77%**.
4. **Seed replay buffer** демонстрациями (данные для критика) + **мягкое
   дообучение SAC**: низкий `log_std` (−2) и малый `ent_coef` (0.02), иначе
   стохастичность SAC разрушает точную BC-политику.
